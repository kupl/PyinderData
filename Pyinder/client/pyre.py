...
def _run_mine_command(
    ...
    return commands.check.run(configuration, check_arguments, mine=True)
...
@pyre.command()
@click.pass_context
def mine(context: click.Context) -> int:
    """
    Runs a one-time type check of a Python project.
    """
    return _run_mine_command(context.obj["arguments"])
